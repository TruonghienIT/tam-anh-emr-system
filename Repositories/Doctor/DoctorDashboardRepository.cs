    ...
    public class DoctorDashboardRepository : RepositoryBase
    {
        ...
        public async Task<Dictionary<string, int>> GetDiseaseStatisticsAsync()
        {
            ...
            WHERE user_id = @userId";
                ...
                // 2. Truy vấn thống kê bệnh (Lấy Top 10 bệnh phổ biến nhất để UI chart không bị tràn)
                string query = @"
            SELECT TOP 10
                d.disease_name AS DiseaseName,
                COUNT(mr.id) AS Count
            FROM medical_records mr
            INNER JOIN diseases d ON mr.icd_code = d.icd_code
            ...
            ORDER BY Count DESC";
                ...
                using (var command = new SqlCommand(query, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            string diseaseName = reader["DiseaseName"] != DBNull.Value
                                ? reader["DiseaseName"].ToString()
                                : "Chưa xác định";
                            int count = Convert.ToInt32(reader["Count"]);
