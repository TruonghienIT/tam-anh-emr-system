...
namespace TamAnh_EMR_System.ViewModel
{
    ...
    public class RegisterPatientViewModel : ViewModelBase
    {
        ...
        {
            // Combine LastName + FirstName → name (Vietnamese convention: Họ Tên)
            string fullName = $"{Patient.LastName?.Trim()} {Patient.FirstName?.Trim()}".Trim();

            // Combine address parts → single address field
            ...
            return new Patients
            {
                // Id will be generated by PatientRepository.AddAsync
                Name = fullName,
                Dob = Patient.DateOfBirth ?? DateTime.Today,
                Gender = Patient.Gender,
                Address = fullAddress,
                Phone = Patient.Phone?.Trim(),
                Email = Patient.Email?.Trim(),
                // Fields not in the current form (can be added in future steps):
                IdCard = null,
                BloodType = null,
                Allergies = null,
        ...
        private void ClearForm()
        {
            ...
            // Re-notify marital status bindings
